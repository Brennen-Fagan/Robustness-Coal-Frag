...
folder_prefix = 'Data';
file_prefix = 'MixedKernel'; %Does not appear to be used.
...
for d = 1 : length(dirs2)
    ...
    disp(['number of files: ', length(files)])
    for f = 1 : length(files)
        ...
        try
            ...
            cols = 0;
            nonz = 0;
            for n = 1 : length(lnames)
                sizes = size(loaded.(lnames{n}));
                rows = rows + sizes(1) - skip_rows;
                cols = max(cols, sizes(2));
                nonz = nonz + nnz(loaded.(lnames{n}));
            end
            stitch = spalloc(rows, cols, nonz); %As recommended by MATLAB.
            
            %Stitch Together: Perform Joining.
            sindex = 1;
            for n = 1 : length(lnames)
                sizes = size(loaded.(lnames{n}));
                stitch((sindex : (sindex + sizes(1) - skip_rows - 1)),...
                       (1 : sizes(2))) = ...
                       loaded.(lnames{n})(1 + skip_rows:end, :);
                sindex = sindex + sizes(1) - skip_rows;
            end
            
            %Save stitched result.
            save(fullfile(file_part_1, file_name_s), 'stitch');
